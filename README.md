# Теория по Информатике
<a name="navigation"><h2>Навигация</h2></a>

- [Задание 1](#number1)
- [Задание 2](#number2)
- [Задание 3](#number3)
- [Задание 4](#number4)
- [Задание 5](#number5)
- [Задание 6](#number6)
- [Задание 7](#number7)
- [Задание 8](#number8)
- [Задание 9](#number9)
- [Задание 10](#number10)
- [Задание 11](#number11)
- [Задание 12](#number12)
- [Задание 13](#number13)
- [Задание 14](#number14)
- [Задание 15](#number15)
- [Задание 16](#number16)
- [Задание 17](#number17)
- [Задание 18](#number18)
- [Задание 19 | 20 | 21](#number192021)
- [Задание 22](#number22)
- [Задание 23](#number23)
- [Задание 24](#number24)
- [Задание 25](#number25)
- [Задание 26](#number26)
- [Задание 27](#number27)
- [Полезные модули и функции](#usefullFuncs)
- [itertools](#itertools)
- [Полезные методы](#usefulMethods)
    - [1. Методы str](#strMethods)
    - [2. Методы list](#listMethods)
    - [3. Методы int](#intMethods)
- [Все нужные функции Exel](#allNeedExelFuncs)
- [Паттерны функций](#FuncPatterns)


<a name="number1">
    <h2>Задание 1(БУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

1. Расписываем кол-во соседей для каждой вершины в таблице
2. Ищем уникальные и расписываеv все остальные относительно них
3. Если уникальных нет то берём любую точку и расписываем все возможные варианты
![Пример решения](./screenshots/numer1.png)
<br>


<a name="number2">
    <h2>Задание 2(БУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

- Теория:
![Функции в Питоне](./screenshots/algFuncsPython.png)

### Пример:
 ```python
 print("x y w z")
for x in range(2):
    for y in range(2):
        for w in range(2):
            for z in range(2):
                p1 = (x<=y)
                p2 = (y==w)
                p3 = (x or z)
                p4 = (p3==w)

                f = ((p1 or p2) and p4)
                if f == 1: print(x, y, w, z)
 ```
 или
 ```python
from itertools import product
print("x y w z")
for x, y, w, z in product(range(2), repeat=4):
    p1 = (x<=y)
    p2 = (y==w)
    p3 = (x or z)
    p4 = (p3==w)

    f = ((p1 or p2) and p4)
    if f == 1: print(x, y, w, z)
 ```
<br>


<a name="number3">
    <h2>Задание 3(БУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

```
=впр(ячейка; диапазон; ячейка выводах; 0)

#четвёртый параметр отвечает за приблизительное значение(0-нет, 1-да)
#так как ответ должен ТОЧНЫМ ВСЕГДА 0!!!!!
```
#### Ключевые формулировки:
- **ПРИБЫЛЬ** == сумма(продаж) - сумма(поставок)
- **ВЫРУЧКА** == сумма(проданного)
- на сколько увеличилось кол-во упаковок == СУММА( упаковок поступления ) - СУММА( упаковок продажи )
- сколько килограммов было продано == СУММА(упаковок продаж) * (кол-во в уваковке)
- сколько килограмм поступило == СУММА(упаковок поступление) * (кол-во в упаковке)
- сколько рублей потребовалось магазинам для закупки == СУММА(упаковокпоступление) * (цена/шт)
- сколько рублей выручили магазины от продажи == (кол-во упоковок продаж) * (цена) -->  ***СУММИРОВАТЬ СТОЛБЕЦ***
- определите общую стоимость продусктов, поставленных за указанный период с ..., в магазиниы ... == (кол-во упаковок поступления) * (цена) -->  ***СУММИРОВАТЬ СТОЛБЕЦ***
- определите общий вес (в кг)..., поступившего в магазины == (кол-во упаковок поступления) * (кол-во в упаковке) --> ***СУММИРОВАТЬ СТОЛБЕЦ***
<br>


<a name="number4">
    <h2>Задание 4(БУ)(2 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

#### - Для решения задания нужно расписать дерево, и в зависимости от вопроса распределить символы:
Условия:
```
Только цила из набора => все цисла набора должны иметь код
Все буквы русского алфавита => должна быть ветка, где буду все ненужные буквы
```
Дальше перебирать возможные варианты.

<br>


<a name="number5">
    <h2>Задание 5(БУ)(4 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>


#### - Перевод в систему счистления
```python
def p(x):
    s =''
    while x!=0:
        s+=str(x%'основание')
        x//='основание'
    return s[::-1]
```

### Пример программы:
```python
def p(x):
    s =''
    while x!=0:
        s+=str(x%3)
        x//=3
    return s[::-1]

def f(x):
    xx = p(x)

    if x%3==0: xx = '1'+xx+'02'
    else: xx= xx + p((x%3)*4)

    return int(xx, 3)


for n in range(10_000):
    r = f(n)
    if r < 100: print(n)
```
<br>


<a name="number6">
    <h2>Задание 6(БУ)(4 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
Номер решаеться в Кумире
Для решения надо переписать команды и ВНИМАТЕЛЬНО ПРОЧИТАТЬ ВОПРОС!!!!
> КЕГЭ - 27182

#### Код Задания:
![Задание 7](./screenshots/numbrer6_task.png)
#### Код Кумира:
![Задание 7](./screenshots/number6_code.png)
#### Картинка:
![Задание 7](./screenshots/number6_pic.png)
1. В поле (1) снимае галочку и выставляем значение 1.0
2. Если надо в поле (2) убераем черепашку
3. Условие в задания: кол-во точек ВНУТРИ ПЕРЕСЕЦЕНИЯ хотябы 2-ух фигур, ВКЛЮЧАЯ ТОЧКИ НА ЛИНИЯХ
4. => Ответ 10
<br>


<a name="number7">
    <h2>Задание 7(БУ)(5 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>


![Задание 7](./screenshots/number7.png)
<br>


<a name="number8">
    <h2>Задание 8(БУ)(4 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>


### 1. Обычный код:
```python
from itertools import product
с = 0
for i in product('КЛЕЙ', repeat=6):  #(алфавит, длина строки)
	i = ''.join(i)  #первращаем tuple в строку
	if i.count('Й') <= 1:  #условие
		if i[0] != 'Й' and i[-1] != 'Й':
			if i.count('ЕЙ')==0 and i.count('ЙЕ')==0:
				c += 1
print(c)
```

### 2. Условие: не начинакется/заканчивается с гласных/согласных букв:
```python
al ='ДГИАШЭ'; c = 0
for i1 in 'ДГШ':
    for i2 in al:
        for i3 in al:
            for i4 in al:
                for i5 in 'ИАЭ':
                    f=i1+i2+i3+i4+i5
                    c+=1
print(c)
```
<br>


<a name="number9">
    <h2>Задание 9(БУ)(6 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>


### Важный момент:
```python
#кол-во чисел без повторов
len(set(ls))
```

### Пример:
```python
c = 0 #счётчик
for s in open('9.txt'):
    ls=  list(map(int, s.split())) #считываем строку в массив
    lp=[]; ln=[] #массив с повторяющимися; неповторяющимися числами
    for a in ls:
        if ls.count(a)==3: lp.append(a)
        elif ls.count(a)==2: lp.append(a)
        else: ln.append(a)

    if len(set(lp))==2 and len(lp)==5:
        p2 = 1 if max(lp)>max(ln) else 0
        if p2: c+=1
print(c)
```
<br>


<a name="number10">
    <h2>Задание 10(БУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
Решается в ворде
<br>


<a name="number11">
    <h2>Задание 11(ПУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
### Формулы:
![Пример решения](./screenshots/number_11_formuli.png)
### Пример:
![Пример решения](./screenshots/number_11.png)
<br>


<a name="number12">
    <h2>Задание 12(ПУ)(6 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
Решается руками - https://vkvideo.ru/video-228926280_456239561
<br>

<a name="number13">
    <h2>Задание 13(ПУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
- Теория
```python
from ipaddress import *

#все возможные ip, без широковещательных
net = ip_network('узел/маска',0)
#маска может быть как 255.255.254.0 так и 27

for ip in net:
    b = f"{ip:b}" #перевод ip адреса в двоичную
    a1=b[:8] #откет 1
    a2=b[8:16] #откет 2
    a3=b[16:24] #откет 3
    a4=b[24:] #откет 4
```
- Нахождение маски для двух узлов
```python
from ipaddress import *

for mask in range(9, 30):
    net1 = ip_network(f'ip_узла1/{mask}', 0)
    net2 = ip_network(f'ip_узла2/{mask}', 0)
    if net1.network_address == net2.network_address:
         print(mask)
```
- Нахождение маски для узла и адреса
```python
from ipaddress import *

for mask in range(9, 30):
    net = ip_network(f'ip_узла/{mask}', 0)
    if net.network_address == 'ip_адрес':
         print(mask)
```

- Примеры
> Определите наибольший IP-адрес данной сети, который может быть присвоен компьютеру. В ответе укажите сумму числовых значений октетов найденного IP-адреса. Если узел 146.180.173.153 и сетевая маска 255.192.0.0
```python
from ipaddress import *
net =ip_network('146.180.173.153/255.192.0.0',0)
print(net[-2]) #146.191.255.254
```
Ответ: 846

> Сеть задана IP-адресом 172.16.160.0 и маской сети 255.255.240.0. Сколько в этой сети IP-адресов, для которых количество единиц в их двоичной записи кратно 2?
```python
from ipaddress import *
net =ip_network('172.16.160.0/255.255.240.0',0)
c = 0
for ip in net:
    b = f"{ip:b}"
    if b.count('1')%2==0: c+=1
print(c)
```
Ответ: 2048

>Сеть задана IP-адресом одного из входящих в неё узлов 192.168.12.207 и сетевой маской 255.192.0.0. Найдите наибольший в данной сети IP-адрес, в котором одинаковое количество нулей и единиц в двоичной записи.
```python
from ipaddress import *
net =ip_network('192.168.12.207/255.192.0.0',0)
for ip in net:
    b = f"{ip:b}"
    if b.count('1')==b.count('0'): c=ip
print(c)
```
Ответ: 192.191.254.0

> КЕГЭ № 19748
```python
from ipaddress import *
ans = 10**100
for mask in range(30, 8, -1):
    net1 = ip_network(f'157.220.185.237/{mask}', 0)
    net2 = ip_network(f'157.220.184.230/{mask}', 0)
    if net1.network_address == net2.network_address:
        c = 0
        for ip in net1:
            b = f"{ip:b}"
            if b.count('1')==15:
                c+=1
        ans = min(ans, c)
        print(ans)
```
Ответ: 9
<br>


<a name="number14">
    <h2>Задание 14(ПУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

## Функции:
- Взятие алфавита:
```python
from string import *
alphabet = (digits + ascii_uppercase + ascii_lowercase)[:основание]  #строка
#или
alphabet = sorted('0123456789QWERTYUIOPASDFGHJKLZXCVBNMqwertyuiopasdfghjklzxcvbnm')[:основание] #массив
```
- Функция перевода для 3,4,5,6,7,9 системы счисления:
```python
def pere(x):
	s = ''
	while x > 0:
		s += str(x%основание)
		x //= основание
	return s[::-1]
```
- Функция для перевода >10 систему счисления
```python
from string import *
os = 36 #основание
alphabet = (digits + ascii_uppercase + ascii_lowercase)[:os]

def pere(x):
    s=''
    o=36 #основание
    while x>0:
        s+= alphabet[x%o]
        x//= o
    return s[::-1]
```
- Базовые функции:
```python
xx = bin(x)[2:] #перевод двоичную
xx = oct(x)[2:] #перевод в восмиричную
xx = hex(x)[2:] #перевод в шеснацетиричную

sy = 2 #система счисления
int(x, sy) #переаод из в десятичную из системы счисления<=36
```

## **Примеры:**
### 1. База:
```python
al = '0123456789ABC'

for x in al:
    for y in al:
        f = int(f'8{x}78{y}', 13)+ int(f'79{x}{y}7', 18)
        if f%9==0: print(f/9)
```

### 2. Матеметическое выражение:
```python
#КЕГЭ №23752
from string import *

def pere(x):
   s=''
   while x!=0:
      s+=al[x%27]
      x//=27
   return s[::-1]

c = 0
al = (digits + ascii_uppercase)[:27]

f = 2*2187**2020 + 729**2021 - 2*243**2022\
    + 81**2023 - 2*27**2024 - 6561
f = pere(f)

for i in al[10:]:
   c+= f.count(i)

print(c)
```
или
```python
#КЕГЭ №23752
f = 2*2187**2020 + 729**2021 - 2*243**2022 + 81**2023 - 2*27**2024-6561
c = 0

while f>0:
    if (f%27)>9: c+=1
    f//=27

print(c)
```

### 3. Система счисления больше 36-ой:
```python
#КЕГЭ №20284
from string import *
al = (digits + ascii_uppercase + ascii_lowercase)

def todec(x:str):
   x = x[::-1]; os = 42
   ans = 0
   for a in range(0, len(x)):
      ans+= al.index(x[a])*os**a
   return ans

for x in digits:
   f1 = todec(f'J569{x}')
   f2 = todec(f'1{x}IA')
   f =f1+f2

   if f%155==0: print(f//155, x)
```
<br>


<a name="number15">
    <h2>Задание 15(ПУ)(3 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
### 1. Тип с координатной прямой:
![Законы](./screenshots/algLaws15.png)\
    Алгоритм Ручками:\
    1.1  Раскрыть функцию по законам\
    1.2  Расписать отрезки\
    1.3 Подставить 0/1 в соответсвии с отрезком\
    1.4 Посчитать длинну в соответствии с заданием
### 2. Поразрядная конъюнкция:
```python
def f(x, a):
    p1 = x&1097==0
    p2 = x&2047!=0
    p3 = x&a!=0
    return p1 <=(p2 <= p3) # выражение

for a in range(1, 1000):
    #массив со всеми значениями
    s = [f(x, a) for x in range(1, 3000)]
    #all всегда
    if all(s): print(a); break
```
### 3. Координатная плоскость
```python
#РЕШУ ЕГЭ №13745
def f(x, y):
    p1 = (x<=9)
    p2 = (x*x<=a)
    p3 = (y*y<=a)
    p4 = (y<=9)
    return (p1 <= p2)and(p3<=p4)

r = range(1, 500)
for a in range(1, 500):
    ls = [f(x, y) for x in range(1, 1_000) for y in range(1, 1_000)]
    if all(ls): ans.append(a)

print(max(ans))
```
### 4. Разное
```python
#КЕГЭ №14658
def ddel(x, xx):
    return x%xx==0

def f(x, a):
    p1 = ddel(x ,12)
    p2 = not(ddel(x, 42))
    p3 = (x+a)>=4096

    return (p1 <= p2) or p3

for a in range(1, 10_000):
    s = [f(x, a) for x in range(1, 10_000)]

    if all(s): print(a);break
```
```python
#КЕГЭ №1345
def f(x):
    p1 = not(x%3==0)
    p2 = x not in [48, 52, 56]
    p3 = abs(x-50)<=7
    p4 = 29<=x<=47
    p5 = x&a==0

    return ((p1 and p2) <= (p3<=p4) or p5)

for a in range(1, 1_000):
    s = [f(x) for x in range(1, 1_000)]
    if all(s): print(a); break
```
```python
#КЕГЭ №5368
def pod(n, m):
    return n%m==0

def f(x):
    p1 =pod(x, 17)
    p2 = not(pod(x, 53))
    p3 = not(a<90_000_000-x)

    return ((p1 <= p2) or p3)

for a in range(90_000_000, 1, -1):
    s = [f(x) for x in range(1, 100_000)]
    if all(s): print(a)
```
<br>


<a name="number16">
    <h2>Задание 16(ПУ)(5 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

### 1. Просто переписать:
```python
from sys import setrecursionlimit
setrecursionlimit(10000) #лимит рекурсии

def f(n):
    if n<20: return n
    else: return (n-6)*f(n-7)

print((f(47872)-290*f(47865))/f(47858))
```

### 2. Через lru_cache:
 ```python
 #РЕШУ ЕГЭ №76713
from functools import lru_cache
from sys import setrecursionlimit
setrecursionlimit(10000)

@lru_cache(None) #кэширует результаты рекурсии
def f(n):
    if n ==0: return 0
    if n>0 and n%2==0: return f(n//10)+n%10
    else: return f(n//10)

c=0
for n in range(4*10**7, 9*10**7 + 1):
    s = f(n)
    if s == 0: c+=1
print(c)
 ```

### 3. Универсальный, через массивы:
```python
ls = [0]*2040  #заполняем массив до наших значений нулями для того, чтоб существовали индексы

for n in range(2025, len(ls)): #перезаписываем индексы после 2025
	f[n] = 1  #команда в задании при опр условии

for n in range(2024, -1, -1):  #перезаписываем индексы от 2024 до -1 (чтоб 0 тоже был)
	f[n] = n-f[n+2]-f[n+4]  #команда в задании при опр условии

print(f[20]+f[25])  #выводим ответ
```
<br>


<a name="number17">
    <h2>Задание 17(ПУ)(14 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

- Важный момент:
```python
ls = [int(x) for x in open("17.txt")] #чтение файла

#два идущих подряд элемента последовательности
for x in range(len(ls)-1):
    s = [ls[i], ls[i+1]]
    #или
    s = ls[i:i+2]

#два различных элемента последовательности
for x in range(len(ls)-1):
	for y in range(x+1, len(ls)):
		s = [ls[x], ls[y]]
```
### Примеры программ:
```python
#КЕГЭ - 23757
ls = [int(x) for x in open("23757.txt")]
pre = 10**100; c = 0; ma = -10**100

for a in ls:
    if 9<a<100:
        pre = min(pre, a)

for i in range(len(ls)-1):
    s = [ls[i], ls[i+1]]
    cc = 0

    for a in s:
        if 9<a<100: cc+=1

    if cc == 1 and sum(s)%pre==0:
        c+=1; ma = max(ma, sum(s))

print(c, ma)
```
```python
#КЕГЭ 14653
ls = [int(x) for x in open('17.txt')]
mi = 10**100; pre2 = -10**100; c = 0 #инициализируем переменые большими числами
                                     #чтоб навереняка сработало


ls1 = [x for x in ls if (abs(x)%17==0 and x>0)]; ls1.sort()
pre1 =  ls1[0] + ls1[1] #сумма двух минимальных положительных
                        #элементов последовательности кратных 17


for a in ls:
    if abs(a)%100 == 69:
        pre2 = max(pre2, a) #максимальный элемент последовательности, оканчивающийся на 69


for i in range(len(ls)-3):
    s = ls[i:i+4]

    ccc2 = 1; ccc1 = ccc3 = 0
    cc1 = cc2 = 0

    for a in s:
        if 99<abs(a)<1_000: cc1+=1 #кол-во трёхзначных чисел
        if abs(a)%18==0: cc2+=1 #кол-во ичсел кратных 18
        ccc1 += a #сумма четвёрки
        ccc2 *= a #произаедение элементов

    if cc1 == 2 and cc2 == 1 and\
       ccc1%pre1==0 and ccc2<=pre2**2:
        c+=1;  mi = min(mi, ccc1**2)

print(c, mi)
```
<br>


<a name="number18">
    <h2>## Задание 18(ПУ)(8 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
> КЕГЭ №28763
Алгоритм:
1. Выделяем точку старта и точки конца\
![18_1](./screenshots/18_1.png)\
2. Выделяем мёртвые зоны\
![18_2](./screenshots/18_2.png)\
3. Выделяем ограниченные ячейки\
![18_3](./screenshots/18_3.png)
4. Комируем таблицу справа и удаляем значения\
![18_4](./screenshots/18_4.png)\
5. В ячейку  W2 вставялем формулу =МАКС(W1;V2)+B2 и продлеваем до AO20\
![18_5](./screenshots/18_5.png)\
6. Удаляем мёртвые зоны\
   Выделяем все зелёные области, заканчивая в W1, пишем формулу =V1+B1 и Cntr+Enter\
   Выделяем все синие области, заканчивая в V2, пишем формулу =A2+V1 и Cntr+Enter\
   В оранжевые ячейки свтавляем =AE7+J8\
![18_6](./screenshots/18_6.png)\
7. Выбераем максимальную из сумм в жётых ячейках -> это ответ 1
8. Cntr+F заменить МАКС на МИН ищем минимальное -> это ответ 2\
Ответ: 2575 466


<br>


<a name="number192021">
    <h2>Задание 19 | 20 | 21 (БУ | ПУ | ВУ) (6 | 8 |11 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

### 1. одна куча камней:
```python
def f(s "stones", m "moves"):
    if s<>"условие выйгрыша": return m%2==0
    if m == 0: return 0
    #ходы для каждой кучи
    h = [f(s-+"ход1", m-1), f(s-+"ход2",m-1), f(s-+"ход3", m-1)]
    return any(h) if m%2!=0 else all(h)

r = range("камни в начальный момент")
print('19)', [s for s in r if f(s,2)])
print('20)', [s for s in r if (not(f(s,1)) and f(s,3))])
print('21)', [s for s in r if (not(f(s,2)) and  f(s,4))])
```

### 2. две кучи камней:
```python
def f(a "куча 1", b "куча 1", m "moves"):
    if a+b<>"условие выйгрыша": return m%2==0
    if m == 0: return 0
    #ходы для каждой кучи
    h = [f(a+1, b, m-1), f(a, b+1, m-1), f(a*2, b, m-1), f(a, b*2, m-1)]
    return any(h) if m%2!=0 else all(h)

c = 2 #в первой куче
r = range("камни в начальный момент")
print('19)', [s for s in r if f(с, s, 2)])
print('20)', [s for s in r if (not(f(с, s, 1)) and f(с, s, 3))])
print('21)', [s for s in r if (not(f(с, s, 2)) and f(с, s, 4))])
```

- !!! при неудачном ходе:
```python
#поледний return меняется ТОЛЬКО для номера с неудачным ходом
return any(h) if m%2!=0 else any(h)
```
<br>


<a name="number22">
    <h2>Задание 22(ПУ)(7 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>
Решается не кодом, в exel (анлак)
<br>


<a name="number23">
    <h2>Задание 23(ПУ)(8 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

### Шаблон:
```python
from sys import setrecursionlimit
setrecursionlimit(5000)

def f(x: int, y: int):
	if x>y: return 0     #обязательно
	if x == y: return 1  #обязательно
	if x == 'избегающее число': return 0
	if x<y: return f(x+1, y) + f(2*x+1, y) #складывание вариантов f(команда_испольнителя ,y)

    print(f('начальное_число', 'обязательное_число') * f('обязательное_число', 'конечное_число'))

```
<br>

<a name="number24">
    <h2>Задание 24(ВУ)(18 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

### 1. Метод вложенный циклов:
```python
m = 0
for l in range(len(s)): #левая граница
    for r in range(l+m, len(s)): #правая граница
        c = s[l:r+1] #срез по границам
```
```python
s = open('1.txt').readline()

m = 0
for l in range(len(s)):
    for r in range(l+m, len(s)):
        c = s[l:r+1]

        if c.count('C')>2 or c.count('D')>2: #условие для оптимизации
            break
        if c.count('C')<=2 and c.count('D')<=2: #условие задания
            m = max(m, len(c))

print(m)
```
### 2. Метод вложенный циклов (для минимума):
```python
m = 1000 #начальное число
for l in range(len(s)):
    for r in range(l+m, l, -1):
        c = s[l:r+1]
```
```python
s = open('24_9169.txt').readline()

m = 1000 #начальное число
for l in range(len(s)):
    for r in range(l+m, l, -1):
        c = s[l:r+1]

        if (c.count("BAD")+c.count("FAT"))>3:
            break
        if (c.count("BAD")+c.count("FAT"))==3:
            m = min(m, len(c))
print(m)
```
### 3. Регулярки
Символы:
[ABC] - один из символов A, B или С
[A-Z] - один из символов от A до Z
[^CDE] - не C, не D, не T
<br>
Кол-во символов:
{3} - ровно три символа
{4, 8} - от 4 до 8
"*" - ноль повторений или больше
"+" - один повторений или больше
<br>
Перечисления:
AB|CD|DE - AB или CD или DE
<br>
Групировка:
(  ) - запоминающая групиировка
(?:  ) - незапоминающая группировка
<br>
Основное:
(?=( регулярка )) - для перебора каждого символа

```python
#КЕГЭ 26551
from re import findall

s = open('24.txt').readline()
pat = r"(?=(  [1-9ABCD][0-9ABCD]*[02468AC]  ))".replace(' ', '') #регулярка под число
# [1-9ABCD] - цифра от 1 до D
# [0-9ABCD]* - цифры от 0 до D >=0 раз
# [02468AC] - чётная цифра
ma = -1

for i in findall(pat, s):
    ma = max(len(i), ma)
print(ma)
```
```python
#КЕГЭ 20813
from re import findall

s = open('24.txt').readline()
pat = r"(?=(  (?:[789][0789]*|0) (?:[-*] (?:[789][0789]*|0) )+  ))".replace(' ', '')
# (?:[789][0789]*|0) - число либо 0
# (?:[-*] (?:[789][0789]*|0) )+ - знак+число >=1 раз
ma = -1

for a in findall(pat, s):
    ma = max(len(a), ma)

print(ma)
```
<br>

<a name="number25">
    <h2>Задание 25(ВУ)(20 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

- Фукция нахождения простого числа:
```python
def is_prime(x):
	for a in range(2, int(x**0.5) + 1):
		if x%a==0:return False
	return True
```
- Фукция нахождения множества делителей
```python
def dels(x):
	se = set()
	for d in range(1, int(x**0.5)+1):
		if x%d==0:
        se.add(d); se.add(x//d)
	return se
```
- Функция нахождения множества простых делителей
```python
def pr_m(x):
    ls = list()
    d = 2

    while d <= x**0.5:
        while x%d==0:
            x //= d
            ls.append(d)
        d+=1

    if x>1: ls.append(x)
    return ls
```

### **Примеры:**
### 1. Маски:
```python
from fnmatch import fnmatch

for i in range(0, 10**10, 'число на которое должно делиться'):
	if fnmatch(str(i), "переписанная маска"):
		print(i, i//'число на которое должно делиться')
```
```python
#РЕШУ ЕГЭ 55821
from fnmatch import fnmatch
for i in range(0, 10**8+1, 273):
    if fnmatch(str(i), "12??36*1"):
        print(i, i//273)
```
```python
#КЕГЭ 22432
from fnmatch import fnmatch
for i in range(84318, 10**12+1, 84318):
    x = str(i)
    if fnmatch(x,"5*7?") and len(set(x))==len(x):
        print(i, i//84318)
```
### 2. Делители
```python
#КЕГЭ 26557
def prime(x): #Фукция нахождения простого числа
    for a in range(2, int(x**0.5)+1):
        if x%a==0: return False
    return True

def dells(x): #Фукция нахождения множества делителей
    se = set()
    for d in range(2, int(x**0.5)+1):
        if x%d==0:
            dd = x//d
            if prime(d): se.add(d) #проверяем простое личисло
            if prime(dd): se.add(dd) #проверяем простое личисло

    return se if se else 0 #если делителей нет, то равно нулю


k = 0
for i in range(7_800_000+1, 10_000_000):
    m = dells(i)

    if m!=0:
        m1 = len(m) #кол-во простых делителей
        M = max(m)+min(m)

    if k == 5: break #

    if M%100==63 and M%m1==0:
        print(i, M)
        k += 1
```
> Ответ:\
> 7800610 780063\
> 7801042 8463\
> 7801312 1863\
> 7801916 8163\
> 7802032 69663
<br>

### 3. Несколько простых - ???

<a name="number26">
    <h2>Задание 26(ВУ)(35 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

### Гайд как вставлять в Exel
1. Копируем файл из txt и всавляем в Exel
![Пример решения](./screenshots/guideStep1.png)
2. Выделяем столбец A:A и переходим в Данные ->  Текст по столбцам
![Пример решения](./screenshots/guideStep2.png)
3. Выбираем нужный символ-разделитель
![Пример решения](./screenshots/guideStep3-1.png)
![Пример решения](./screenshots/guideStep3-2.png)
![Пример решения](./screenshots/guideStep3-3.png)
4. Создаём новый лист и снова вставляем данные
![Пример решения](./screenshots/guideStep4.png)
5. Готово
![Пример решения](./screenshots/masha.jpg)



### 1. Системный администратор
```python
#Номер РЕШУ ЕГЭ - 27423
S = 8200; N = 970
ls = [int(x) for x in open('263.txt')]
ls.sort() #сортируем, чтоб было легче

su = 0 #сумма для ответа 1
ans1 = 0 #количество для ответа 1

#прохидимся и считаем ответ 1
for i in range(N):
    if su + ls[i] <= S: #проверяем, чтоб сумма помещалась на диск
        su += ls[i]
        ans1 += 1 #количество файлов
        max_v = ls[i] #максимальный объём файла

free_space = S - su #вычисляем сколько места осталось
pot_max_v = free_space + max_v #вычисляем примерный объём наибольшего файла

for i in range(N):
    if ls[i] > pot_max_v: #если файл больше, то берём предыдущий
        ans2 = ls[i-1]

print(ans1, ans2)
```
> Ответ: 568, 50

### 2 Скидка на товары
```python
#РЕШУ ЕГЭ 33105
N = 1000; file = '261.txt'
#   N = 6; file = 'demo.txt'
ls = [int(x) for x in open(file)]
ls.sort()

no_skidka = [] #массив для товаров без скидки
good_skidka = [] #массив для подходящих под скидку товаров
bad_skidka = [] #массив для дорогих товаров для скидки
a = len( [x for x in ls if x<=100] )/2 # кол-во товаров подходящиее
                                        #под условие скидки

for x in ls:
    if x<= 100: no_skidka.append(x)
    elif a > 0: good_skidka.append(x); a-=1
    else: bad_skidka.append(x)


ans1 = sum(no_skidka) + sum(good_skidka)*0.7 +\
       sum(bad_skidka) #ответ 1
ans2 = max(good_skidka) #ответ 2

#округляем ответ 1
print(ans1, ans2)
```
> Ответ: 459678, 543

### 3. Грузы
>РЕШУ ЕГЭ 33198 - Решение Exel и Calc аналогичные

1. сортируем столбец A:A по возрастанию
2. находим числа от 200 до 210, выписываем в соседний столбец
3. В ячейку C1 прописываем формулу =B1-СУММ(B184:B196)
![Пример решения](./screenshots/33198ExelStep2-3.png)
4. В ячейку C2 прописываем формулу =C1-A2 и протягиваем до отрицательного числа
![Пример решения](./screenshots/33198ExelStep4.png)
Начинаем заменять по условию
5. Значение 37 это сколько у нас осталос место, то есть мы можем заменить 123 на 123+37=160
6. подзодящее значение только 157, тогда отсаётся ещё 3, тогда мы можем замениеть 123 на 126
7. Ближайшее 125 => заменяем и остаётся ещё 1, тогда  можем заменить 122 на 123
8. Остаток ноль =>  ответ 2 = 10000, а товет 1 = 111-1(первая ячека пуста)+13(числа от 200 до 210) = 123
Ответ: 123, 10000

### 4. Модификации
> Номера по типу (РЕШУ ЕГЭ - 38960) лучше решать в exel ручками
...

### 5. Лёгкая матрёшка
>КЕГЭ - 21910
```python
#КЕГЭ 5228
#N = 5; file = 'demo.txt'
N = 6200; file = '26.txt'
ls = [int(x) for x in open(file)]
ls.sort(reverse = True)

curr = ls[0] #нынешная труба
ans = [curr] #массив с трубами

for x in range(1, len(ls)):
    if curr - ls[x] >= 8:
        curr = ls[x]
        ans.append(curr)

ans1 = len(ans) #ответ 1
ans2 = ans[-1]  #ответ 2
print(ans1, ans2)
```
Ответ: 369, 123

### 6. Средняя матрёшка
Начнём со 2-го ответа, т.к. легче найти
```python
#Номер РЕШУ ЕГЭ - 48447
#Ответ 2
N = 10000;
ls = [int(x) for x in open('262.txt')]
ls.sort(reverse =  True) #разворачиваем список

curr = ls[0] #число от которого будем оталкиваться
ans2 = 1 #макс кол-во контейнеров

for i in range(1, N): #идём от 1, т.к. curr
    if curr - ls[i] >= 5: #условие вложения
        curr = ls[i] #сдвигаем curr
        ans2+=1 #нашли подходящий блок -> +1

print(ans2)
```
```python
#Номер РЕШУ ЕГЭ - 48447
#Ответ 1
N = 10000; ans2 = 1767
ls = [int(x) for x in open('262.txt')]
ls.sort(reverse =  True)

ans1 = 0
banned = [] #индексы контейнеров, которые уже упакованы

while True:
    for i in range(0, N): #находим curr для N-прохода
        if banned.count(i) == 0: #проверяем нет взят ли индекс контейнера
            curr = ls[i] #находим точку входа, баним и выходимы
            banned.append(i)
            break

    for i in range(1, N): #(улучшенный код ответа 2)
        if banned.count(i) == 0: #проверяем нет взят ли индекс контейнера
            if (curr - ls[i])>=5:
                curr = ls[i]
                banned.append(i)
    ans1 +=1

    if len(banned) == N: #когда всё перебанили выходим из while
        break

print(ans1)
```
> Ответ: 17, 1767

### 7. Сложная матрёшка
> КЕГЭ - 5988
```python
#ответ 2
n = 1500
ls= []
for x in open('26.txt'):
    w,c=x.split()
    ls.append([int(w), c])
ls.sort(reverse=True)
curr = ls[0]

for a in range(1, len(ls)):
    if curr[0]-ls[a][0]>=7:
        curr = ls[a]
        ans2 = ls[a][0]

print(ans2)
```
```python
#ответ 1
n = 1500
ls= []
for x in open('26.txt'):
    w,c=x.split()
    ls.append([int(w), c])

ls.sort()
ans1=[0]*n

for i in range(n):
    m=0
    for j in range(0, i):
        if (ls[i][0] - ls[j][0]>=7 and ls[i][1]!=ls[j][1]):
            m=max(m, ans1[j])
    ans1[i]=m+1

print(max(ans1))
```
Ответ: 628, 4

### 8. Мфц/ячейки
```python
#КЕГЭ - 23283
k = 267; N = 994;
#[[начало приёма, конец приёма], ...]
ls = [ list(map(int, x.split())) for x in open('мфц.txt') ]
ls.sort()


windows = [0]*k #окна
ans1 = 0 #кол-во человек может подойти
ans2 = 0 #последнее окно


for x in ls:
    for i in range(k):
        #смотрим в какое окно может подойти чел
        if x[0] >= windows[i]+1:
            windows[i] = x[1] #записываем время оконцания приёма
            ans1 += 1
            ans2 = i+1 #тк i - индекс, то +1 для номера
            break #чела приняли => дальше проверять не надо

print(ans1, ans2)
```
>Ответ: 793, 2
```python
#КЕГЭ - 19724
k = 230; N = 10000;
#[[начало хранения, конец хранения], ...]
ls = [ list(map(int, x.split())) for x in open('ячейки.txt') ]
ls.sort()


cell = [0]*k
ans1 = 0 #кол-во принятых дронов
ans2 = 0 #последняя ячейка


for x in ls:
    for i in range(k):
        #смотрим в какую ячейку можно положить
        if x[0] >= cell[i]+1:
            cell[i] = x[1] #записываем время оконцания хранения
            ans1 += 1
            ans2 = i+1 #тк i - индекс, то +1 для номера
            break #посылку приняли => дальше проверять не надо

print(ans1, ans2)
```
>Ответ: 1862, 211

### 9. Транспортная лента / Две характеристики
>КЕГЭ - 23765 | Решние Exel

8.0. Копируем содержимое txt в Exel, и в первую строчку прописываем названия колон, что нормально фильтровать\
8.1. В столбце C:C прописываем формулу: =МИН(A2:B2)\
8.2. В столбце D:D прописываем формулу: =ЕСЛИ(C2=A2;"хранение";"годность")\
8.3. Столбец E:E промунеровываем с 1\
![Пример решения](./screenshots/23765ExelStep1-3.png)\
8.4. Сортируем столбец C:C по возрастанию\
8.5. Листаем вниз и последний номер это ответ 1\
![Пример решения](./screenshots/23765ExelStep4-5.png)\
- Так как характеристика последний строки это годность, а по условию:"если минимальное число – срок годности к употреблению, то продукт занимает первое свободное место от конца рейтинга", то ответ на второй вопрос это (кол-во полей срока годности)-1

8.6. Фильтруем столбец D:D по "годность" и копирпуем без первой строки на новый лист\
8.7. Листаем вниз и ((номер последней строки) - 1) это ответ 2\
![Пример решения](./screenshots/23765ExelStep6-7.png)\
Ответ: 564, 444

>КЕГЭ - 23765 | Решние Calc

8.0. Копируем содержимое txt в Calc и удаляем первое значение\
8.1. В столбце C:C прописываем формулу: =МИН(A2:B2)\
8.2. В столбце D:D прописываем формулу: =ЕСЛИ(C2=A2;"хранение";"годность")\
8.3. Столбец E:E промунеровываем с 1\
8.4. Сортируем столбец C:C по возрастанию\
![Пример решения](./screenshots/23765CalcStep1-4.png)\
8.5. Листаем вниз и последний номер это ответ 1\
![Пример решения](./screenshots/23765CalcStep5.png)\
- Так как характеристика последний строки это годность, а по условию:"если минимальное число – срок годности к употреблению, то продукт занимает первое свободное место от конца рейтинга", то ответ на второй вопрос это (кол-во полей срока годности)-1

8.6. Фильтруем столбец D:D по "годность"\
8.7. Выделяем столбец со значениями(cntr + shift + стрелка вниз) и (выбранно - 1) это ответ 2\
![Пример решения](./screenshots/23765CalcStep6-7.png)\
Ответ: 564, 444

### 10. "Контрольные точки" / Один чел - несколько значений
>КЕГЭ - 23383 | Решние Exel

0. Копируем содержимое txt в exel, и в первую строчку прописываем названия колон, что нормально фильтровать\
1. фильтруем столбцы A:A и B:B по возростанию\
![Пример решения](./screenshots/23383ExelStep1-1.png)\
![Пример решения](./screenshots/23383ExelStep1-2.png)\
2. удаляем дубликаты\
![Пример решения](./screenshots/23383ExelStep2.png)\
3. сортирум только столбец B:B по возростанию\
4. Прописываем в С2 1, а в C3 формулу: =ЕСЛИ(И(B3=B2;A3-A2=1);C2+1;1)\
![Пример решения](./screenshots/23383ExelStep3-4.png)\
5. С помощью фильтра оставляем только максимальное значение в столбце C:C\
6. Ответ 1 - 56 (столбец 3); Ответ 2 - 30113(столбец 2)\
![Пример решения](./screenshots/23383ExelStep5-6.py.png)\
Ответ 56, 30113

>КЕГЭ - 23383 | Решние Calc

0. Копируем содержимое txt в Calc, и удаляем первое значение\
1. фильтруем столбцы A:A и B:B по возростанию\
![Пример решения](./screenshots/23383CalcStep1.png)\
2. удаляем дубликаты и копируем результат на новый лист\
![Пример решения](./screenshots/23383CalcStep2-1.png)\
![Пример решения](./screenshots/23383CalcStep2-2.png)\
3. сортирум только столбец B:B по возростанию\
4. Прописываем в С2 1, а в C3 формулу: =ЕСЛИ(И(B3=B2;A3-A2=1);C2+1;1)\
![Пример решения](./screenshots/23383CalcStep3-4.png)\
5. С помощью фильтра оставляем только максимальное значение в столбце C:C\
6. Ответ 1 - 56 (столбец 3); Ответ 2 - 30113(столбец 2)\
![Пример решения](./screenshots/23383CalcStep5-6.png)\
Ответ 56, 30113
<br>

### 11. "Кандидат в отряд"
>КЕГЭ №20815 | Exel

0. Копируем значения в Exel и подписываем стобцы\
![Пример решения](./screenshots/20815ExelStep0.png)\
1. Сортируем в соответствии с заданием. (Сортировка и фильтр>Настраеваемая сортировка)\
![Пример решения](./screenshots/20815ExelStep1.png)\
2. Переходик строчке 1076+1(первая строчка с подписями)\
![Пример решения](./screenshots/20815ExelStep2.png)\
3. Тогда полупроходной балл - 279, проходной - 280\
4. id последний строчки с баллом 280 - ответ 1 => 45539\
![Пример решения](./screenshots/20815ExelStep4.png)\
5. Количество человек с полупроходнымбаллом - ответ 2 => 127\
![Пример решения](./screenshots/20815ExelStep5.png)\
Ответ: 45539, 127

> Решение этого номера аналогично для Cacl, только сортировка находится здесь:
![Пример решения](./screenshots/20815Calc.png)

### 12. Лента
...

<br>

<a name="number27">
    <h2>Задание 27(ВУ)(40 мин)</h2>
</a>

<h100>[Навигация](#navigation)</h100>

- Важный момент:
```python
#растояние можно находить 2-мя способами
ln = ((x2-x1)**2 + (y2-y1)**2)**0.5
# или
from math import dist
ln = dist([x1, y1], [x2, y2])
```

- Функция нахждения центра кластера:
```python
...
#или
def f(k): #работаерт быстрее
    return min(k, key = lambda q: sum(dist(q, p) for p in k))
    #возвраoает массив [x, y]
```

- Алгоритм решения: 25443
1. Открываем Calc и вставляем координаты точек\
2. Проверяем, что ничего не съехало и строим диаграмму с размерами 40x20 см\
3. делим на кластеры\
![Пример кластеров](./screenshots/klasteri_1.png)
![Пример кластеров](./screenshots/klasteri_2.png)
3.1. если не понятно, где находится точка, то возпользуемся методом зоркого глаза\
![Пример кластеров](./screenshots/klasteri_3.1.1.png)\
3.1.1. ставим размер 200x100 см\
3.1.2. находим место, где нам интересна точка\
3.1.3. кликаем 2 раза и смотрим координату точки\
![Пример метода 1](./screenshots/klasteri_3.1.2.png)\
![Пример метода 2](./screenshots/klasteri_3.1.3.png)\
3.1.4. -> получаем кластер 1 при: 4<x<8 and 0.6<y<5.8\

3.2. если кластеры нельзя поделить по прямым, то\
3.2.1. можно отделить их лесинками\
![Пример прямой](./screenshots/klasteri_3.2.1.png)\
кластер 1 -> (5<=x<6 and 0<y) or (x<5 and y>-1), else кластер 2\
3.2.2. можно отделить их с мопощью прямых\
![Пример прямой](./screenshots/klasteri_3.2.2.png)\
точка 1(2,-2) точка 2(10,8) -> k = 5/4 -> функция g(x) = (5/4)*x-4.5 (математика пригодилась)\
проверяем водя точки с шагом 0,1:
![Пример прямой](./screenshots/klasteri_3.2.3.png)\
тогда если y>g(x), то кластер 2\
тогда если y<g(x), то кластер 3\
4. прописываем условие, не забывая про аномалии\
5. в соответствии с условием даём ответ\
5.1 если есть какая-то непонятная формулировка, скорее всего вначале задания это прописано\
6. Переапроверяем ВСЁ(функции, кол-во точек в кластерах, название файлов в каждом файле, ...)

## Примеры решения
> КЕГЭ №18676

Файл А:
![КЕГЭ №18676 A](./screenshots/klasteri_3.2.1.png)
```python
from math import dist
k1 = []; k2 = []

def f(k):
    return min(k, key = lambda q: sum(dist(q, p) for p in k))

for a in open('a.txt'):
    x, y = map(float, a.replace(',','.').split())

    if (5<=x<6 and 0<y) or (x<5 and y>-1): k1.append([x,y])
    else: k2.append([x,y])

x1,y1 = f(k1)
x2,y2 = f(k2)

px = int(((x1+x2)/2)*100_000)
py = int(((y1+y2)/2)*100_000)

print(px, py)
```

Файл Б:
![КЕГЭ №18676 Б](./screenshots/klasteri_3.1.1.png)
![КЕГЭ №18676 Б](./screenshots/klasteri_3.2.3.png)
```python
from math import dist
k1 = []; k2 = []; k3 = []

def f(k):
    return min(k, key = lambda q: sum(dist(q, p) for p in k))

for a in open('b.txt'):
    x, y = map(float, a.replace(',','.').split())

    if (4<x<8 and 0.6<y<5.8): k1.append([x,y])
    elif y > ((5/4)*x-4.5): k2.append([x,y])
    elif y < ((5/4)*x-4.5): k3.append([x,y])

x1,y1 = f(k1)
x2,y2 = f(k2)
x3,y3 = f(k3)

px = int(((x1+x2+x3)/3)*100_000)
py = int(((y1+y2+y3)/3)*100_000)

print(px, py)
```
> КЕГЭ №25443

Файл А:
![КЕГЭ №25443_A](./screenshots/25443_A.png)
```python
from math import dist
k1 = []; k2 =[]; anom = 0

def f(k):
    return min(k, key = lambda q: sum(dist(q, p) for p in k))

for a in open('a.txt'):
    x, y = map(float, a.replace(',','.').split())

    if x>0 and 0<y<10: k1.append([x,y])
    elif x>0 and 10<y<30: k2.append([x,y])
    else: anom+=1

c1 = f(k1)
c2 = f(k2)

p1 = int(dist(c1,c2)*10_000)
p2 = -10**100
for a in k1: p2 = max(p2, dist(c1,a))
for a in k2: p2 = max(p2, dist(c2,a))

print(p1, int(p2*10_000))
```

Файл Б:
![КЕГЭ №25443_Б](./screenshots/25443_B.png)
```python
from math import dist
k1 = []; k2 =[]; k3 =[]; anom = 0

def f(k):
    return min(k, key = lambda q: sum(dist(q, p) for p in k))

for a in open('b.txt'):
    x, y = map(float, a.replace(',','.').split())

    if 20<x<30 and 0<y<20: k1.append([x,y])
    elif 10<x<20 and 11<y<20: k2.append([x,y])
    elif 10<x<20 and 0<y<11: k3.append([x,y])
    else: anom+=1

x1, y1 = f(k1)
x2, y2 = f(k2)
x3, y3 = f(k3)

Qx = int(((x1+x2+x3)/3)*10_000)
Qy = int(((y1+y2+y3)/3)*10_000)

print(Qx, Qy)
```

> КЕГЭ №21599
```python
```

<br>


<a name="usefullFuncs">
    <h1>Полезные модули и функции</h1>
</a>

<h100>[Навигация](#navigation)</h100>

1. setrecursionlimit() - задаёт ручной лимит рекурсии
```python
from sys import setrecursionlimit
setrecursionlimit(10_000) # нельзя устанавливать слишком большое значение
						  # тк есть ограничения операционной системы
							#на личном опыте было проверено 300_000 - работает
```
2. lru_cache - декоратор, который сохраняет значения функции и их не надо считать второй раз, что ускоряет работу
```python
from functools import lru_cache

@lru_cache(None)
def f(n):
	if n == 1: return 1
	if n > 1: return f(n - 1) - 2*g(n - 1)
```
>    Следует использовать когда:
>    - Функции, которые выполняются многократно или долго.
>    - Функции, которые часто вызываются с одними и теми же аргументами.
>    - Функции, которые требуют больших вычислительных затрат.
3. алфавит из string
```python
from string import *

end = 16 #система счисления 16
s = (digits + ascii_uppercase + ascii_lowercase)[:end]
# s - алфавит с основанием 16
```
>1. digits - все числа
>2. ascii_uppercase - все заглавные буквы
>3. ascii_lowercase - все строчные буквы
4. enumerate - добавляет автоматический счётчик к елементам, возвращая пару-число, значение
```python
numbs = [150, 160, 170]
for index, num in enumerate(numbs, 2):
    print(f"{index}. {num}")
```
> 2. 150
> 3. 160
> 4. 170

<br><br>


<a name="itertools">
    <h1>itertools</h1>
</a>

<h100>[Навигация](#navigation)</h100>

1. **product**(алфавит, repeat=длина строки) - для перебора
```python
for i in product("01234567", repeat=5):
	print(i__class__) #tuple с симолами

	i = i.''join(i)
	print(i.__class__) #строка
```
2. **combinations** - ???
3. **permutations** - ???




<a name="usefulMethods">
    <h1>Полезные методы</h1>
</a>

<h100>[Навигация](#navigation)</h100>

<a name="strMethods">
    <h2>1. Методы str</h2>
</a>

|метод|что делает|
|---|------------|
|s.count('подстрока')|выводит кол-во подстрок в строке|
|s.replace('что заменить', 'на что заменмить', сколько раз)|заменяет символы в строке|
|s.split('параметр')|возвращает массив со строками рабитые по параметру|
|s.index('подстрока')|выводит первый индекс подстроки|
|s.find('подстрока')|выводит первый индекс в стороке|
|s.strip()|убирает пробелы в начале и конце строки|
|s.islower() <br> s.isupper()|проверяет чтоб все символы в строке были строчными <br> проверяет чтоб все символы в строке были заглавными|
|s.lower() <br> s.upper()|превращает все символы в строчные <br> превращает все символы в заглавные|
|s.swapcase()|меняет (все ЗГЛАВНЫЕ на строчные) и (все строчные на ЗАГЛАВНЫЕ)|

<br>

<a name="listMethods">
    <h2>2. Методы list</h2>
</a>

|метод|что делает|
|---|------------|
|ls.append(a) <br> a += [15]|добавляет в список элемент|
|ls.insert(in, a)|добасляет объект на опр индекс|
|ls.remove(a)|удаляет первый по эл a|
|ls.pop(in)|удаляет эл по индексу|
|ls.sort() <br> ls1 = sorted(ls)|сортирует список|
|ls.sort(reverse=True) <br> ls1 = sorted(ls)[::-1]|сортирует списк по убыванию|
|ls.index(a)|индекс первого эл a|

<br>

<a name="intMethods">
    <h2>3. Методы int</h2>
</a>

|метод|что делает|
|---|------------|
|i.\_\_ceil\_\_()|Округляет число в большую сторону|
|i.\_\_floor\_\_()|Округляет число в меньшую сторону|

<br>

<a name="allNeedExelFuncs">
    <h1>Все нужные функции Exel</h1>
</a>

<h100>[Навигация](#navigation)</h100>

|функция|что делает|
|-----|---------------|
|МИН(число) <br> МАКС(число)|Мин значение <br> Макс значение|
|ЕСЛИ(условие, если да, если нет)|обычное условие|
|И() <br> ИЛИ()|Обычные функции и / или|
|СУММ(диапозон) <br> СУММЕСЛИ(диапозон; условие)|Сумма значений в списке <br> Сумма значений в списке при выполнении условия|
|СЧЁТ(диапозон) <br> СЧЁТЕСЛИ(диапозон; условие)|Подсчёт элементов в списке <br> Подсчёт элементов в списке при выполнении условия|
|СРЗНАЧ(диапозон) <br> СРЗНАЧЕСЛИ(диапозон; условие)|Среднее арифметическое чисел <br> Среднее арифметическое чисел при выполнении условия|
|НАИБОЛЬШИЙ(диапозон; по счёту(k)) <br> НИМЕНЬШИЙ(диапозон; по счёту(k))|Возвращает k-ое наибольшее число из списка <br> Возвращает k-ое наименьшее число из списка (k – аргумент)|
|ЕЧЁТН(число) <br> ЕНЕЧЁТ(число)|ИСТИНА, если число четное, иначе ЛОЖЬ <br> ИСТИНА, если число нечётное, иначе ЛОЖЬ|
|ОСТАТ(число; делитель)|Остаток при делении числа на второй аргумент|
|ЦЕЛОЕ(число)|Берёт только целую часть числа|
|ОКРУГЛВВЕРХ(число, разряды) <br> ОКРУГЛВНИЗ(число, разряды)|Округляет число в большую сторону до k-ого разряда <br> Округляет число в меньшую сторону до k-ого разряда|
|ВПР(клетка; диапазон; клетка вывода; 0)|Ищет совпадения в левом крайнем столбце таблицы и <br> возвращает значение из указанного столбца той же строки|


<a name="FuncPatterns">
    <h1>Способы определения</h1>
</a>

<h100>[Навигация](#navigation)</h100>

- Проверка на полный квадрат:
```python
def f(x):
	if x>=0 and x**0.5%1==0:
		return True
```

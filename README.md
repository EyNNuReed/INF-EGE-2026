...
- [Все нужные функции Excel](#allNeedExelFuncs)
- [Может пригодиться](#FuncPatterns)
...
Решается руками - [Принцип решения](https://vkvideo.ru/video-228926280_456239561)
...
for mask in range(8, 32):
    net = ip_network(f'ip_узла/{mask}', 0)
    if net.network_address == ip_address('ip_сети'):
        print(mask)
```
...
def f(s "stones", m "moves"):
    if s<>"условие выигрыша": return m%2==0
    if m == 0: return 0
...
def f(a "куча 1", b "куча 1", m "moves"):
    if a+b<>"условие выигрыша": return m%2==0
    if m == 0: return 0
    #ходы для каждой кучи
...
<h100>[Навигация](#navigation)</h100>
Есть два типа 22-ых задач:
1) минимальное время - решаетася с помощью впр
2) максимальная продолжительность - решаетася с помощью рисования и перебора

### ВПР
> КЕГЭ №28941 (впр)
1. [Распределяем процессы по группам](#guide_excel)(0; 1 число; 2 числа; 3 числа)
2. раскрашиваем зоны (красный - id a | жёлтый - для впр | зелёный - для времени окончания)\
![Пример решения](./screenshots/28941_1.png)
3. для "ID процесса (ов) A" со значением 0 в зелёную секцию прописываем: =Время выполнения процесса B (мс)
4. для 1-го числа прописываем формулу: =ВПР(C5;$ A:$ I;9;0) и протягиваем на всю желтую часть с цифрами
5. в зелёной секции прописываем формулу: =МАКС(F5:H5)+B5\
![Пример решения](./screenshots/28941_2.png)
6. удаляем формулу впр там где нет чисел в красной зоне
7. и максимальное число в зелёной секции - это ответ\
![Пример решения](./screenshots/28941_3.png)
> Ответ: 35

### Перебор
> КЕГЭ №27632

[Видео Решение](https://vk.com/video-205865487_456240541?t=1h9m45s)
...
for i in range(987654321, 0, -1):
    s = pr_m(i)
    if k == 5: break
    if len(s)==13 and str(sum(s)).count('1'):
        print(i, max(s)); k+=1
```
>Ответ:\
987654144 160751\
...
###
<a name="guide_excel">
    <h3>Гайд как вставлять в Excel</h3>
</a>

1. Копируем файл из txt и всавляем в Excel\
...
### 3. Грузы
>РЕШУ ЕГЭ 33198 - Решение Excel и Calc аналогичные

[Гайд как вставлять в excel](#guide_excel)
...
### 4. Начало и конец
> Номер КЕГЭ - 28945
0. если дана продолжительность, то делаем столбец "конец" c формулой =A1 + B1
1. [вставляем данные в excel](#guide_excel) и именем столбцы, чтобы не запутаться\
![Пример решения](./screenshots/28945_1.png)
2. в первой строчке дуюлируем конец, а во второй прописываем формулу: =если(начало>=первой строчки; конец; первая строчка)\
![Пример решения](./screenshots/28945_2.png)
3. копируем правый столбец и на новый лист, удаляем дубликаты и выделяем все значения\
![Пример решения](./screenshots/28945_3.png)
4. количество - это ответ 1
5. ответ 2 по ситуации
...
>КЕГЭ - 23765 | Решние Excel

0. [Копируем содержимое txt в Excel](#guide_excel), и в первую строчку прописываем названия колон, что нормально фильтровать
...
0. [Копируем значения в Excel](#guide_excel) и подписываем стобцы\
...
### 12. Минимальный ряд / кинотетр
```python
#КЕГЭ №20910
n = 9992; m = 99906; k = 6661; file = '26.txt'
pole = [m+1]*(k+1) #минимальный ряд для каждого места

for a in open(file):
    row, mesto = map(int,a.split())
    pole[mesto] = min(pole[mesto], row) #для места узнаем какой самый первый ряд


m = []
for i in range(1, k):
    r = min(pole[i], pole[i+1])-1 #минус 1, тк берём перед занятым рядом
    m.append(r)
...
for i in range(1, k):
    r =min(pole[i], pole[i+1])-1
    if r==ans1: ans2 = [i, i+1] #ищем пару кресел в ряду
...
1. **product**(алфавит, repeat=длина строки) - для перебора всех возможных вариантов
```python
...
```
2. **combinations**(алфавит, длина) - Генерирует все возможные комбинации переданной длинны из элементов алфавита. (элементы не повторяются)
```python
...
```
3. **permutations**(алфавит, длина) - Генерирует все возможные перестановки длины переданной длинны из элементов алфавита. (порядок важен)
```python
...
<a name="allNeedExelFuncs">
    <h1>Все нужные функции Excel</h1>
</a>

...
1. В столбце C:C прописываем формулу: =МИН(A2:B2)
2. В столбце D:D прописываем формулу: =ЕСЛИ(C2=A2;"хранение";"годность")
3. Столбец E:E промунеровываем с 1\
![Пример решения](./screenshots/23765ExelStep1-3.png)
4. Сортируем столбец C:C по возрастанию
5. Листаем вниз и последний номер это ответ 1\
...
6. Фильтруем столбец D:D по "годность" и копирпуем без первой строки на новый лист
7. Листаем вниз и ((номер последней строки) - 1) это ответ 2\
...
6. Фильтруем столбец D:D по "годность"
7. Выделяем столбец со значениями(cntr + shift + стрелка вниз) и (выбранно - 1) это ответ 2\
